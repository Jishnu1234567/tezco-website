...
const TECH_ICONS = [
  ...
  { label: "Redis",      color: "#FF4438", svg: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M10.9 4.8L7.5 6.4l3.4 1.6 3.4-1.6-3.4-1.6zm-4.9 2.3l3.4 1.6v3.2L6 10.3V7.1zm4.9 4.8l-3.4-1.6v3.2l3.4 1.6 3.4-1.6v-3.2l-3.4 1.6zm4.9-3.2L12.3 10v3.2l3.5-1.6V7.7zM0 2.7v18.6L12 24l12-2.7V2.7L12 0 0 2.7z"/></svg>` },
  ...
  { label: "GraphQL",    color: "#E10098", svg: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M14.051 2.751l4.935 2.85c.816-.859 2.173-.893 3.032-.077.859.816.893 2.173.077 3.032-.245.258-.552.446-.889.543v5.701c1.138.341 1.784 1.553 1.443 2.691-.341 1.138-1.553 1.784-2.691 1.443-.648-.194-1.155-.701-1.349-1.349H6.391c-.341 1.138-1.553 1.784-2.691 1.443-1.138-.341-1.784-1.553-1.443-2.691.194-.648.701-1.155 1.349-1.349V9.1c-1.138-.341-1.784-1.553-1.443-2.691.341-1.138 1.553-1.784 2.691-1.443.337.101.644.289.889.547l4.935-2.85c-.208-1.173.575-2.294 1.748-2.502 1.173-.208 2.294.575 2.502 1.748.037.204.037.413 0 .617z"/></svg>` },
  ...
  { label: "Vite",       color: "#646CFF", svg: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M0 0l10.594 18.489L24 0h-4.703L10.594 13.011 4.703 0H0z"/></svg>` },
...
const MarqueeRow = React.memo(({ reversed, duration }: { reversed: boolean; duration: string }) => {
  ...
  return (
    ...
    >
      <div
        className={reversed ? "mq-rev" : "mq-fwd"}
        style={{ display: "flex", width: "max-content", animationDuration: duration }}
      >
        {[0, 1].map(copy =>
          icons.map((item, i) => (
...
const SliderCard = React.memo(({
  ...
  return (
    <div
      ...
      className="slider-card absolute"
      style={{
        width: "clamp(200px,48vw,310px)",
        height: "clamp(290px,60vw,470px)",
        transform: `translate3d(${offset * 82}%,0,0) rotateY(${offset * -35}deg) scale(${active ? 1 : 0.82})`,
        opacity: abs > 1 ? 0 : 1,
        zIndex: active ? 30 : 20 - abs,
        // Off-screen cards: NEVER intercept pointer or touch events
        pointerEvents: abs > 1 ? "none" : "auto",
        cursor: active ? "default" : "pointer",
      }}
    >
      <div className={`w-full h-full rounded-[2rem] overflow-hidden border-2 transition-colors duration-500 ${active ? "border-cyan-500/40 shadow-[0_0_60px_-10px_rgba(34,211,238,0.35)]" : "border-white/10"}`}>
        <img
          src={card.img}
          alt={card.title}
          loading="eager"
          ...
          style={{
            ...
            filter: active ? "brightness(0.72)" : "brightness(0.45) saturate(0.7)",
        ...
        {active && (
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent">
            <div className="absolute bottom-8 left-7 right-7">
              <span className="text-cyan-400 text-[10px] font-black tracking-[0.3em] uppercase block mb-1">{card.sub}</span>
              <h3 className="font-black text-white uppercase leading-none mb-3" style={{ fontSize: "clamp(18px,5vw,34px)" }}>{card.title}</h3>
...
export default function ModernEffectsPage() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      else setTimeout(() => setIsLoading(false), 350);
    };
    requestAnimationFrame(tick);
  }, []);

  // ── Road parallax — compositor-only ──────────────────────────────────────
  useEffect(() => {
    if (isLoading) return;
    const onScroll = () => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        const scale = Math.max(0, 1 - window.scrollY / window.innerHeight);
        roadRef.current.style.transform = `scaleY(${scale})`;
      });
    };
  ...
  // ── Slide change with debounce lock ──────────────────────────────────────
  ...
  // ── Touch: imperative listeners so we control passive/active per-axis ────
  //
  //  THE FIX for "scroll stuck after cards":
  //
  //  React's onTouchStart/onTouchMove are always passive in React 17+ (or
  //  passive:true by default in many browsers). That means we cannot call
  //  preventDefault() from them — but we also cannot block native scroll.
  //  Unfortunately, `touch-action: pan-y` alone doesn't stop the sticking
  //  because the 3D-transformed card children can confuse the browser's
  //  hit-testing, causing it to think a touch started on an element that
  //  wants horizontal pan.
  //
  //  Solution: attach a SINGLE non-passive touchmove listener directly to the
  //  slider section via useEffect. Inside it we:
  //    1. Measure dx vs dy on the FIRST move event after touchstart.
  //    2. If vertical → do nothing (native scroll runs freely).
  //    3. If horizontal → preventDefault() to take over and trigger a slide.
  //  We never preventDefault() on vertical touches, so scroll is NEVER
  //  blocked, in any direction, before or after the cards section.
  // ─────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const onEnd = (e: TouchEvent) => {
      ...
      if (Math.abs(dx) > 48) changeSlide(dx > 0 ? 1 : -1);
    };

    el.addEventListener("touchstart", onStart, { passive: true });
    el.addEventListener("touchmove",  onMove,  { passive: false }); // must be non-passive to allow preventDefault
  ...
  return (
    <div className="relative bg-[#050505] text-white overflow-x-hidden" style={{ fontFamily: "'Archivo',sans-serif" }}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .mq-fwd { will-change:transform; animation:mq-fwd linear infinite; }
        .mq-rev { will-change:transform; animation:mq-rev linear infinite; }
        @media (hover:hover){ .mq-fwd:hover,.mq-rev:hover{ animation-play-state:paused } }
        @media (prefers-reduced-motion:reduce){ .mq-fwd,.mq-rev{ animation:none!important } }

        /* Slider card: only compositor props in transition, isolated paint layer */
        .slider-card {
          will-change: transform, opacity;
          transform-style: preserve-3d;
          /* 
            contain:layout style paint
            Each card is fully isolated — repaints on one card can't cascade
            to the rest of the page, preventing the layout-flush stutter that
            causes the scroll-stuck sensation.
          */
          contain: layout style paint;
          transition: transform 0.75s cubic-bezier(0.22,1,0.36,1), opacity 0.4s ease;
        }

        /* Road element shrinks from bottom purely on the compositor thread */
        .road-el {
      ...
      {/* ── LOADER ───────────────────────────────────────────────────────── */}
      {isLoading && (
        <div className="fixed inset-0 z-[9999] bg-white flex flex-col justify-end p-6 md:p-10 overflow-hidden">
          <div
            className="absolute inset-y-0 left-0 bg-black"
            style={{ width: `${progress}%`, transition: "width 32ms linear" }}
          />
          <span
            className="relative z-10 font-black leading-none tracking-tighter select-none"
            style={{ fontSize: "clamp(72px,22vw,220px)", color: progress < 50 ? "#000" : "#fff", transition: "color 0.2s" }}
          >
            {Math.round(progress)}%
          </span>
        </div>
      ...
      <section className="h-screen flex flex-col justify-end items-center relative overflow-hidden bg-black">
        ...
        <div
          ref={roadRef}
          className="road-el bg-white mix-blend-difference absolute bottom-0 w-full"
          style={{ height: "100vh" }}
        />
      </section>
      ...
      <section className="bg-white py-16 md:py-20 px-6 md:px-10 min-h-screen relative overflow-hidden">
        <div className="mt-[40vh] md:mt-[20vw] space-y-1 md:space-y-3 text-right relative z-10">
          {["BUILD BY", "TECHNICAL", "EXPERTS"].map(t => (
            <h3 key={t} className="font-black text-black uppercase" style={{ fontSize: "clamp(32px,9vw,110px)", lineHeight: 0.78 }}>{t}</h3>
          ))}
        </div>
      </section>
      ...
      <section
        ...
        /*
          touch-action:pan-y is a browser-level hint that this element prefers
          vertical panning. It's a hint only — our imperative touchmove listener
          above is the actual enforcement mechanism. Both layers working together
          guarantee vertical scroll is NEVER interrupted.
        */
        style={{ touchAction: "pan-y" }}
      >
        {/* Arrow nav — wrapper is pointer-events:none so it doesn't eat touches */}
        <div className="absolute inset-0 flex items-center justify-between px-4 md:px-8 z-50 pointer-events-none">
          {([[-1, ChevronLeft], [1, ChevronRight]] as [number, React.ElementType][]).map(([dir, Icon]) => (
            <button
              key={dir as number}
              onClick={() => changeSlide(dir as number)}
              className="pointer-events-auto p-3 md:p-4 rounded-full bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/15 active:scale-90 transition-all"
        ...
        <div
          ...
          style={{ height: "clamp(320px,72vw,520px)", perspective: "1200px" }}
        >
          ...
          {sliderData.map((_, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: 3,
                background: i === activeIndex ? "#22d3ee" : "rgba(255,255,255,0.25)",
                border: "none",
                padding: 0,
                cursor: "pointer",
